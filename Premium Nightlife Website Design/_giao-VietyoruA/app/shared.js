/* nightlife.hn — dữ liệu & logic dùng chung cho prototype bấm-được.
   PHƯƠNG ÁN A · Premium Elegant (than đen + Gold). Đặt window.NL trước khi support.js render. */
window.NL = (function () {
  ...
  // ---------- gold (điểm nhấn chủ lực Phương án A) ----------
  var GOLD = 'linear-gradient(135deg,#f4e3b4,#d4b26a 55%,#b6924a)';
  ...
  // tim: bật = hồng neon (đặc trưng A, dùng cực hạn chế); tắt = line mờ
  function heart(on) { return on ? 'https://img.icons8.com/ios-filled/100/E0729E/like.png' : 'https://img.icons8.com/ios/100/C5C0B6/like.png'; }

  // ---------- style helpers (Phương án A) ----------
  function chip(on) { return 'cursor:pointer;font-size:12.5px;border-radius:16px;padding:7px 13px;font-weight:600;white-space:nowrap;transition:all .14s;' + (on ? 'background:' + GOLD + ';color:#241a0a;' : 'background:rgba(255,255,255,.05);color:#c5c0b6;'); }
  function pill(on) { return 'display:inline-flex;align-items:center;justify-content:center;cursor:pointer;font-size:13px;border-radius:18px;padding:8px 16px;font-weight:600;white-space:nowrap;transition:all .14s;' + (on ? 'background:' + GOLD + ';color:#241a0a;border:1px solid transparent;' : 'background:rgba(255,255,255,.04);color:#c5c0b6;border:1px solid rgba(255,255,255,.1);'); }
  function pillSm(on) { return 'display:inline-flex;align-items:center;justify-content:center;cursor:pointer;font-size:11.5px;border-radius:15px;padding:7px 12px;font-weight:600;white-space:nowrap;' + (on ? 'background:' + GOLD + ';color:#241a0a;border:1px solid transparent;' : 'background:rgba(255,255,255,.04);color:#c5c0b6;border:1px solid rgba(255,255,255,.1);'); }
  function seg(on) { return 'flex:1;text-align:center;cursor:pointer;font-size:13px;border-radius:9px;padding:9px;font-weight:600;transition:all .14s;' + (on ? 'background:' + GOLD + ';color:#241a0a;box-shadow:0 1px 3px rgba(0,0,0,.4);' : 'color:#8c8679;'); }
  function tab(on) { return 'cursor:pointer;padding-bottom:12px;font-weight:' + (on ? '700' : '500') + ';color:' + (on ? '#e3c27e' : '#8c8679') + ';border-bottom:2px solid ' + (on ? '#d4b26a' : 'transparent') + ';transition:all .14s;'; }
  ...
  // badge trạng thái — tông tối + gold (Phương án A)
  function bkBadge(s) { return { 'new': { label: 'Mới', color: '#e3c27e', bg: 'rgba(212,178,106,.14)' }, done: { label: 'Hoàn tất', color: '#7fd6a6', bg: 'rgba(60,160,110,.16)' }, cancelled: { label: 'Đã hủy', color: '#e89aa0', bg: 'rgba(190,70,84,.18)' } }[s]; }
  function cpBadge(s) { return { holding: { label: 'Đang giữ chỗ', color: '#e3c27e', bg: 'rgba(212,178,106,.14)' }, used: { label: 'Đã sử dụng', color: '#9b958a', bg: 'rgba(255,255,255,.06)' }, expired: { label: 'Hết hạn', color: '#e89aa0', bg: 'rgba(190,70,84,.18)' } }[s]; }
