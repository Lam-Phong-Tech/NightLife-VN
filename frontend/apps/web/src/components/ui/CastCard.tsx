  ...
  const content = (
    <div onClick={onClick} className="card" style={{ background: 'var(--vy-surface)', borderRadius: '16px', overflow: 'hidden', boxShadow: 'var(--vy-shadow-card)', cursor: 'pointer', border: '1px solid var(--vy-border)' }}>
      <PlaceholderMedia src={cast.img} alt={cast.name || 'Cast'} label="Ảnh cast" style={{ height: '118px', position: 'relative' }}>
        ...
        <span onClick={(e) => { e.preventDefault(); if (onFavClick) { onFavClick(e); } else if (cast.fav) { cast.fav(e); } }} style={{ position: 'absolute', top: '8px', right: '8px', width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg,var(--vy-favorite),var(--vy-favorite-strong))', border: '1px solid var(--vy-favorite-border)', boxShadow: '0 10px 24px -14px var(--vy-favorite-glow)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
