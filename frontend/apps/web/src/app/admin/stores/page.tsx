  ...
  const fetchStores = async () => {
    ...
    try {
      ...
      if (status === 403 || /quyền|forbidden/i.test(e?.message ?? '')) {
        setFetchError('Bạn không có quyền xem danh sách Quán. Vui lòng liên hệ Admin để được cấp quyền.');
      } else {
  ...
  return (
    <div className="nl-admin-page" data-screen-label="Admin · Venues" style={{ padding: '22px 26px 44px', minHeight: '100vh', background: '#0c0c0f' }}>
      ...
      <div className="nl-admin-list-toolbar" style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap', marginBottom: '16px' }}>
        <div className="nl-admin-inline-search max-md:w-full" style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '10px', padding: '8px 13px', width: '250px' }}>
      ...
      {venueSel && typeof document !== 'undefined' ? createPortal((
        <div className="nl-admin-store-drawer-overlay" style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
          ...
          <div className="scw nl-admin-drawer nl-admin-store-drawer" style={{ position: 'absolute', right: 0, top: 0, height: '100vh', width: '576px', maxWidth: '96vw', overflow: 'auto', background: '#131218', borderLeft: '1px solid rgba(212,178,106,.18)', boxShadow: '-30px 0 60px -30px rgba(0,0,0,.8)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="nl-admin-store-drawer-header" style={{ position: 'sticky', top: 0, zIndex: 2, padding: '19px 26px', background: '#131218', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <div>
