  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      if (status === 403 || /quyền|forbidden/i.test(e?.message ?? '')) {
        setFetchError('Bạn không có quyền xem danh sách đặt chỗ. Vui lòng liên hệ Admin để được cấp quyền.');
      } else {
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
            ...
            <div className="scw nl-admin-drawer" style={{ position: 'absolute', right: 0, top: 0, height: '100vh', width: '428px', maxWidth: '92vw', overflow: 'auto', background: '#131218', borderLeft: '1px solid rgba(212,178,106,.18)', boxShadow: '-30px 0 60px -30px rgba(0,0,0,.8)' }}>
              ...
              <div style={{ padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                {bk.tourName && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '18px', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,.05)' }}><span style={{ fontSize: '12.5px', color: '#8c8679', flex: 'none' }}>Tên tour</span><span style={{ fontSize: '13px', color: '#f3f0ea', fontWeight: 500, textAlign: 'right' }}>{bk.tourName}</span></div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '18px', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,.05)' }}><span style={{ fontSize: '12.5px', color: '#8c8679', flex: 'none' }}>Quán</span><span style={{ fontSize: '13px', color: '#f3f0ea', fontWeight: 500, textAlign: 'right' }}>{bk.store?.name || bk.store}</span></div>
                ...
                {bk.status === 'REQUESTED' && (
                  <>
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <span onClick={() => !isActionLoading && handleUpdateStatus(bk.id, 'CHECKED_IN')} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '13px', fontWeight: 700, color: '#241a0a', background: 'linear-gradient(135deg,#f4e3b4,#d4b26a 55%,#b6924a)', padding: '13px', borderRadius: '11px', cursor: isActionLoading ? 'not-allowed' : 'pointer', opacity: isActionLoading ? 0.6 : 1 }}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>Đã check-in</span>
