...
describe('Booking and Discount Flows Backend Integration Challenger Suite', () => {
  ...
  describe('2. VIP, Member, and Guest tier validations for Admin Coupons', () => {
    ...
    it('rejects a Regular Member user from claiming an Admin Coupon targeted at VIP tier', async () => {
      prisma.adminCoupon.findFirst.mockResolvedValue(adminCouponMock as any);

      await expect(
        service.claimAdminGlobalCouponForMember('admin-coupon-vip', {
          id: 'user-member',
          role: 'USER',
          tier: 'MEMBER',
        } as AuthenticatedUser),
      ).rejects.toThrow(
        new UnprocessableEntityException('User tier not eligible for this coupon'),
      );
    });

    it('rejects a Guest user from claiming an Admin Coupon targeted at VIP tier', async () => {
      prisma.adminCoupon.findFirst.mockResolvedValue(adminCouponMock as any);
      prisma.guest.findFirst.mockResolvedValue({ id: 'guest-1' } as any);

      await expect(
        service.claimAdminGlobalCouponForGuest('admin-coupon-vip', {
          phone: '+84901234567',
          displayName: 'Guest User',
        }),
      ).rejects.toThrow(
        new UnprocessableEntityException('Guest not eligible for this coupon'),
      );
    });

    it('blocks booking if user tier does not match the Admin Coupon targetAudience', async () => {
      prisma.store.findFirst.mockResolvedValue({
        id: 'store-target',
        name: 'Target Club',
        slug: 'target-club',
      ...
      // Member (non-VIP) user tries to use the VIP admin coupon during booking
      await expect(
        service.createMemberBooking(
          { id: 'member-4', role: 'USER', tier: 'MEMBER' }, // tier MEMBER!
          {
            storeSlug: 'target-club',
            adminCouponIssueId: 'admin-issue-vip',
            displayName: 'Regular Member',
            phone: '+84901234567',
            scheduledAt: new Date(Date.now() + 86400000 * 2).toISOString(),
            partySize: 2,
        ...
        new UnprocessableEntityException('User tier is not eligible for this admin coupon'),
      );
    });
  });
