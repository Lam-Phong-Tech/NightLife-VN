...
const YT = {
  ...
  club_edm_vietnam: 'x0yNaLNI4c4',          // Club EDM Vietnam
...
function ytThumb(key: YtKey): string {
  return `https://img.youtube.com/vi/${YT[key]}/maxresdefault.jpg`;
}
...
const STORE_HEROES: Record<string, string> = {
  // HCM
  'moonlight-bar':        unsplashImg('1566417713065-a3030c4c0d1d', 1200, 800), // dimly lit bar
  'velvet-club':          unsplashImg('1571266752397-f82a3fe0e0b3', 1200, 800), // nightclub crowd
  'sakura-lounge':        unsplashImg('1540575467537-21c88dd4a6bc', 1200, 800), // japanese lounge
  'golden-voice-ktv':     unsplashImg('1526182096566-f6a70ddeba89', 1200, 800), // karaoke room
  'hanami-dining':        unsplashImg('1414235077428-338989a2e8c0', 1200, 800), // restaurant dining
  'lotus-massage-spa':    unsplashImg('1498534314096-f338dc7c52e5', 1200, 800), // spa tranquil
  // HN
  'crimson-bar':          unsplashImg('1572116469935-1d65e9e3a166', 1200, 800), // craft beer bar
  'neon-club':            unsplashImg('1519671282429-b44b61db6037', 1200, 800), // neon club
...
const CAST_AVATARS: Record<string, string> = {
  ...
  'yuki-sakura-lounge':      unsplashImg('1524504388515-d21f41f840a5', 400, 500), // refined woman
  'hana-sakura-lounge':      unsplashImg('1488426862026-3ee34a7d66ef', 400, 500), // sweet woman
  ...
  'mika-golden-ktv':         unsplashImg('1502767882231-6c2ed47b7ea8', 400, 500), // ktv host
  'rumi-hanami':             unsplashImg('1484863137850-bafde1703ef3', 400, 500), // elegant dining
  'kaori-hanami':            unsplashImg('1464863979621-258859e62245', 400, 500), // graceful woman
  'misaki-crimson':          unsplashImg('1542206395-9feb3edv50c0a', 400, 500), // lively woman
  ...
  'sora-neon':               unsplashImg('1500021804447-2490001b82c4', 400, 500), // cool DJ
  'akari-jade':              unsplashImg('1586297135537-9b5cfd851f64', 400, 500), // sophisticated
  'hikaru-jade':             unsplashImg('1544005313-94ddf0286df2', 400, 500), // warm woman
  'erika-star':              unsplashImg('1514626585935-44b53e79dfd0', 400, 500), // vocalist
  ...
  'aoi-tokyo':               unsplashImg('1529688499-a816c7e048d1', 400, 500), // gentle woman
  'lina-dragon-rooftop':     unsplashImg('1511285560929-80b5a269e25e', 400, 500), // rooftop party
  'mai-dragon-rooftop':      unsplashImg('1504439468489-c8920d796a4c', 400, 500), // dancer style
  'nami-son-tra':            unsplashImg('1562124638-7ff6dd49f5c3', 400, 500), // beach lounge
  ...
  'hana-harbor-ktv':         unsplashImg('1559181567-c3190958d3d4', 400, 500), // warm hostess
  'sumi-opera-spa':          unsplashImg('1436716198405-9debf8b27b29', 400, 500), // spa concierge
  'yuri-opera-spa':          unsplashImg('1573496359142-b8d87734a5a2', 400, 500), // gentle spa
  'sumi-lotus-massage-spa':  unsplashImg('1507297230445-ff7d73e9c792', 400, 500), // massage spa
};
...
export async function seedMedia(
  ...
  for (const s of STORE_LABELS) {
    const heroUrl = STORE_HEROES[s.slug] ?? unsplashImg('1566417713065-a3030c4c0d1d');
    mediaItems.push({
  ...
  const GALLERY_POOL = [
    '1470337458703-4ad1d7ba7fd8', // dark bar crowd
    '1543007631-893bc5f1afed',   // cocktail glasses
    '1528823872057-9c018a07a7ad', // nightclub lights
    '1514362545857-3bc16c4c7d1b', // restaurant interior
    '1550966871-1851f89f7537',   // rooftop view
    '1578683994935-4aa5e9f11e39', // bar counter
    '1556909114-f6e7ad7d3136',   // japanese food
    '1504674900247-0877df9cc836', // dining table
    '1513694203232-719a6b182195', // cocktail making
    '1581091226825-a6a2a5aee158', // lounge seating
    '1551632436-cbf8dd35adfa',   // live music bar
    '1425421598197-76a25c3f55cf', // club dancefloor
  ...
  for (const c of CAST_LABELS) {
    const avatarUrl = CAST_AVATARS[c.slug] ?? unsplashImg('1494790108377-be9c29b29330', 400, 500);
    mediaItems.push({
