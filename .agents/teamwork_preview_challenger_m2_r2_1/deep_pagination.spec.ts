import { NightlifeDataService } from '../../backend/src/nightlife-data/nightlife-data.service';
...
describe('Deep Pagination Stress Test (>60 items)', () => {
  ...
  beforeEach(() => {
    ...
    // Construct 125 total items (42 bills, 42 coupons, 41 bookings)
    const baseTime = new Date('2026-08-05T12:00:00.000Z').getTime();

    // 42 Bills with decreasing timestamps (some identical)
    for (let i = 1; i <= 42; i++) {
      // Offset by 10 minutes per item, but items 10-15 share the exact same timestamp
    ...
    // Mock Prisma findMany logic to simulate DB cursor filter and take limit
    prisma = {
      bill: {
        findMany: jest.fn().mockImplementation((args: any) => {
          ...
          // Apply cursor filter (AND array containing cursor OR conditions)
          if (args.where?.AND) {
  ...
  it('paginates continuously across 125 items (>60 items, 7 pages) without loss, duplication, or ordering bugs', async () => {
    ...
    while (hasMore) {
      ...
      if (pageCount < 7) {
        ...
        // Total items 125 = 6 pages of 20 + 1 page of 5
  ...
  it('handles empty query after dataset exhaustion', async () => {
    // Obtain last cursor from 3rd page where limit is 50
    ...
    // Query again with cursor after end
