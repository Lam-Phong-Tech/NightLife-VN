...
const desktopDir = path.join(process.cwd(), 'tools/Wirefame_Nightlight/app/desktop');
const mobileDir = path.join(process.cwd(), 'tools/Wirefame_Nightlight/app/mobile');
const destDir = path.join(process.cwd(), 'frontend/apps/web/src/app');
...
allFiles.forEach(file => {
  ...
  import { recs, cityTabs, cats, areas, venues, castFilters, castCards, offers, rankListQuan, rankListCast } from '@/lib/mock-data';
  
  export default function Page() {
    const [state, setState] = useState<any>({});
    const forceUpdate = () => setState({...state});
    
    const self = { state, setState, forceUpdate };
    
    ${Object.keys(dRes.lists).length > 0 || Object.keys(mRes.lists).length > 0 ? `
    // Mock data for lists
    ${Array.from(new Set([...dRes.lists, ...mRes.lists])).map(l => {
      if (['recs', 'cityTabs', 'cats', 'areas', 'venues', 'castFilters', 'castCards', 'offers'].includes(l) || l.startsWith('rankList')) {
        return ''; // Imported from mock-data
      }
      if (l === 'vRecs' || l === 'dRecs') {
        return \`const \${l}: any[] = [
          { name: 'Club Lumière', area: 'Tây Hồ', catLabel: 'Bar Lounge', rating: 4.9, img: "url('https://images.unsplash.com/photo-1572116469696-31de0f17cc34?auto=format&fit=crop&w=480&q=70') center/cover", favIcon: 'https://img.icons8.com/ios/100/FFFFFF/like.png', price: '1.2tr', grad: 'linear-gradient(140deg, #d6336c, #7b2d6b)', open: () => window.location.href = '/chi-tiet-quan', fav: (e: any) => { e.preventDefault(); e.stopPropagation(); } },
          { name: 'Sakura Club', area: 'Ba Đình', catLabel: 'KTV', rating: 4.7, img: "url('https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&w=480&q=70') center/cover", favIcon: 'https://img.icons8.com/ios/100/FFFFFF/like.png', price: '2.5tr', grad: 'linear-gradient(140deg, #6d28d9, #4c1d95)', open: () => window.location.href = '/chi-tiet-quan', fav: (e: any) => { e.preventDefault(); e.stopPropagation(); } },
          { name: 'VIP Lounge', area: 'Hoàn Kiếm', catLabel: 'Lounge', rating: 4.8, img: "url('https://images.unsplash.com/photo-1566417713940-fe7c737a9ef2?auto=format&fit=crop&w=480&q=70') center/cover", favIcon: 'https://img.icons8.com/ios-filled/100/d6336c/like.png', price: '1.8tr', grad: 'linear-gradient(140deg, #f59e0b, #b45309)', open: () => window.location.href = '/chi-tiet-quan', fav: (e: any) => { e.preventDefault(); e.stopPropagation(); } },
          { name: 'Tokyo Night', area: 'Ba Đình', catLabel: 'Bar', rating: 4.5, img: "url('https://images.unsplash.com/photo-1542282811-943ef1a67702?auto=format&fit=crop&w=480&q=70') center/cover", favIcon: 'https://img.icons8.com/ios/100/FFFFFF/like.png', price: '900k', grad: 'linear-gradient(140deg, #059669, #064e3b)', open: () => window.location.href = '/chi-tiet-quan', fav: (e: any) => { e.preventDefault(); e.stopPropagation(); } }
        ];\`;
      }
      return '';
    }).join('\n    ')}
